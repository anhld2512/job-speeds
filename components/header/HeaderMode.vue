...
const darkThemeColor = '#1d232a';
const lightThemeColor = '#ffffff';

const currentTheme = ref(process.client ? localStorage.getItem('theme') || 'light' : 'light');
...
const changeTheme = () => {
  ...
  const jobSpeedElement = document.getElementById('job-speed');
  if (jobSpeedElement) {
    jobSpeedElement.setAttribute('data-theme', currentTheme.value);
  }
  if (process.client) {
  ...
  nextTick().then(() => {
    if (process.client) {
      setTimeout(() => {
        ...
        const jobSpeedElement = document.getElementById('job-speed');
        if (jobSpeedElement) {
          jobSpeedElement.setAttribute('data-theme', theme);
        }

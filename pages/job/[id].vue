<template>
    <div class="container h-full overflow-hidden mx-auto top-20">
      <div class="fixed container mx-auto px-5 md:px-0 right-0 left-0 -mt-4 flex w-full items-center justify-between bg-base-100">
        <div @click="myFormApplicantion.showModal()" :class="{ 'w-full md:w-1/2 lg:w-1/3': !isAuthor }" class="btn btn-md m-1 text-start btn-primary rounded">
          <i class="bi bi-heart text-xl"></i> Apply Now
        </div>
        <div v-if="isAuthor" class="btn btn-sm btn-circle btn-accent dropdown dropdown-bottom dropdown-end text-primary items-center">
          <button tabindex="0" class="text-black"><i class="bi bi-gear text-xl"></i></button>
      ...
      <div class="w-full mt-3 md:mt-12 md:shadow-md rounded p-6 md:border md:border-2 rounded-xl h-full overflow-auto">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <span class="badge badge-lg badge-accent mb-2"><i class="bi bi-calendar3 mr-1"></i> {{ $filters.dateStringFormat(currentJob?.dateExpired, "DD/MM/YYYY") }}</span>
          <span class="badge badge-lg badge-neutral mb-2"><i class="bi bi-award mr-1"></i> {{ currentJob?.jobCategory }}</span>
          <span class="badge badge-lg badge-error mb-2"><i class="bi bi-clock-history mr-1"></i> {{ currentJob?.jobTyped }}</span>
        </div>
        ...
        <div class="flex flex-wrap w-full items-center my-1">
          <div class="w-full text-star">
            ...
            <div class="card-actions justify-start flex-wrap mb-2 ml-1">
              <div v-for="(itemSkill, index) in currentJob?.jobSkills" :key="index" class="badge badge-lg badge-outline">{{ itemSkill }}</div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div class="col-span-1 md:col-span-2/3">
            <figure class="w-full h-60 md:h-full skeleton">
              <div class="h-full w-full bg-cover bg-center rounded-lg" :style="{ backgroundImage: `url(${currentJob?.jobImageUrl})` }"></div>
            </figure>
          </div>
        </div>
      </div>
  ...
  const onSubmit = (isVal) => {
    if (isVal) {
      ...
      $modelAPI.jobAPI.updateJobById(currentJob.value._id, currentJob.value)
        .then(result => {
          if (result.data.value.result) {
  ...
  const applications = (isVal) => {
    if (isVal) {
      ...
      triggerToast('success', 'Application successful');
    } else {
      myFormApplicantion.value.close();
      triggerToast('error', 'Please try again');
    }
  };
  </script>
